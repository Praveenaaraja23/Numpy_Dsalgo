...
When The user clicks login button after entering valid Vidya and Kanaka@185
  ...
  Then The user1 be directed to Queue Data Structure Page
   ...
   @TryEditorinputs1valid
    Scenario Outline: The user1 is able to run code in tryEditor for Implementation of Queue in Python page
    Given The user1 is in the tryEditor page for Implementation of Queue page
    When The user1 enter the valid and invalid pythoncode input from sheet <Sheetname> and <RowNumber> in Queue Clicks_run_button
    Then The user1 should able to see output in the console output from excelsheet <Sheetname> and <RowNumber> in Queue
    Examples: 
      |Sheetname  |RowNumber  |
      |phythoncode|         0 |
